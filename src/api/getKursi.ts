...
export async function getKursiFromServ(
  ...
  const data =
    teacher && teacher !== 'all'
      ? allData.filter(item => item['Працівник, який пройшов курсову підготовку'] === teacher)
      : allData.filter(item => item['Працівник, який пройшов курсову підготовку'].length > 2);

  return { data, all };
}

export async function getKursi(
  idSheet: string,
  teacher: string
): Promise<{ data: PageItem[]; all: string[] } | null> {
  try {
    const response = await axios.get(`https://znz16300.github.io/sitedata/data/${idSheet}.json`);
    ...
    const data =
      teacher !== '' && teacher !== 'all'
        ? allData.filter(
            (item: PageItem) => item['Працівник, який пройшов курсову підготовку'] === teacher
          )
        : allData.filter(
            (item: PageItem) => item['Працівник, який пройшов курсову підготовку'].length > 2
          );
